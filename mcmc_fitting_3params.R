...
#### mult, which is the multiiplier for ssortative mixing
...
my_log_lh_func_3params <- function(params){
  ...
  # run the TB model with the input parameters, output will be mortality over 9 age groups
  tb_results = ageStrTBModel_mortality_3params(params$mult, params$beta_o, params$tau, t_year, eqbm)
...
my_log_priors_func_3params <- function(params){
  ...
  for (param_name in names(params)){
    if (param_name == "mult"){  # flat prior
      ...
      y = dunif(x=params[[param_name]], min = 0.002, max = 0.02, log=TRUE)
    }
...
my_proposal_func_3params <- function(params){
  ...
  # standard deviations
  sd=list(mult=2.5, beta_o=0.01, tau=0.002)
