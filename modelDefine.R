...
getParameters<-function(Nclasses, mult, beta1){
  ...
  parameters<-list(
   ...
   # delta=0*73/(0.41*365*70*3),# prob of treatment=0, no treatment case
    ...
    alpha = 0.5, # fractional reduction in force of infection due to recovery from first infection
    rho = 0.3 # proportion of infected people who are infectious. avg over years, see Notifications table , WHO TB report.
  )
  
}

TBmodel_9ageclasses<-function(t,n,parameters){
    ...
    with(as.list(parameters),{
      ...
      # for(a in 1:9){
      #   if(a==1){
      #     dS_udt[a]=(pi*N_tot) - (mu[a]*S_u[a]) - lamda*S_u[a] - ((tau_out[a])*S_u[a])
      #     dL_adt[a]=lamda*S_u[a] + (alpha*lamda*S_r[a]) + (chi*lamda*L_b[a]) - (mu[a]+epsilon[a]+kappa[a])*L_a[a] - ((tau_out[a])*L_a[a])
      ...
      #     dS_rdt[a]= phi*I[a]- ( (alpha*lamda) + mu[a]) * S_r[a] - ((tau_out[a])*S_r[a])
      #   }
      #   if(a%in% 2:8){
      #     dS_udt[a]= - (mu[a]*S_u[a]) - lamda*S_u[a] +(tau_in[a])*S_u[a-1] - (tau_out[a])*S_u[a]
      #     dL_adt[a]=lamda*S_u[a] + (alpha*lamda*S_r[a]) + (chi*lamda*L_b[a]) - (mu[a]+epsilon[a]+kappa[a])*L_a[a] + (tau_in[a])*L_a[a-1] - (tau_out[a])*L_a[a]
      ...
      #     dS_rdt[a]= phi*I[a] - ( (alpha*lamda) + mu[a]) * S_r[a] + (tau_in[a])*S_r[a-1] - (tau_out[a])*S_r[a]
      #   }
      #   
      #   if(a==9){
      #     dS_udt[a]= - (mu[a]*S_u[a]) - lamda*S_u[a] + (tau_in[a])*S_u[a-1]
      #     dL_adt[a]= lamda*S_u[a] + (alpha*lamda*S_r[a]) + (chi*lamda*L_b[a]) - (mu[a]+epsilon[a]+kappa[a])*L_a[a] + (tau_in[a])*L_a[a-1]
      ...
      #     dS_rdt[a]= phi*I[a] - ( (alpha*lamda) + mu[a]) * S_r[a] + (tau_in[a])*S_r[a-1]
      #     
      #   }
      #   
      # }
...
#This changes the disease model parameter alpha=alpha1. Don't confuse with the alpha which is the y-intecept for the time varying beta function
# Bad choice of variable names - can cause confusion down the track
# Too complicated to change everywhere, so keeping it like this for now.
getParameters_timevarybeta_alpha<-function(Nclasses, mult, beta_o, tau, alpha1, gamma, alpha){
  # function that returns the model parameters
  # The model has 9 age classes - Daw 1950 paper
  # The model has demography parameters based on geographic location
  # It has epi parameters based on geography and age
  ...
  parameters<-list(
    ...
    beta_func=beta_func, #the beta_func will create the correct beta_mx inside the ode model
    chi = 0.21, # fractional reduction in the force of infection corresponding to return from late to early latency
    alpha = alpha1, # fractional reduction in force of infection due to recovery from first infection
    rho = 0.3 # proportion of infected people who are infectious. avg over years, see Notifications table , WHO TB report.
  )
...
TBmodel_9ageclasses_timevarybeta<-function(t,n,parameters){
  ...
  with(as.list(parameters),{
    ...
    for(a in 1:9){
      if(a==1){
        #AA: made it closed popln as model had the total population decreasing, with closed popln thetotal popln increases gradually.
        #dS_udt[a]=((mu[a]*N_tot) + (sum(I)*mu_I)) - (mu[a]*S_u[a]) - lamda_vec[a]*S_u[a] - ((tau_out[a])*S_u[a])
        dS_udt[a]=(pi*N_tot) - (mu[a]*S_u[a]) - lamda_vec[a]*S_u[a] - ((tau_out[a])*S_u[a])
        dL_adt[a]=lamda_vec[a]*S_u[a] + (alpha*lamda_vec[a]*S_r[a]) + (chi*lamda_vec[a]*L_b[a]) - (mu[a]+epsilon[a]+kappa[a])*L_a[a] - ((tau_out[a])*L_a[a])
        dL_bdt[a]= kappa[a]*L_a[a] + gamma*I[a] -(mu[a]+nu[a]+(chi*lamda_vec[a]) )*L_b[a] - ((tau_out[a])*L_b[a])
        dIdt[a]= nu[a]*L_b[a] + epsilon[a]*L_a[a] - (mu_I+gamma+phi) * I[a] - ((tau_out[a])*I[a])
